package com.devepos.adt.abaptags.validation;
...
public class TagListValidator {
	...
	public IStatus validate(final boolean rootOnly, final boolean newOnly) {
		return validateTags(this.tagList, rootOnly, newOnly);
	}
		...
		for (final ITag tag : tags) {
			...
			if (tag.getName().contains(">")) { //$NON-NLS-1$
				return new Status(IStatus.ERROR, AbapTagsModelPlugin.PLUGIN_ID,
					NLS.bind(Messages.TagListValidator_InvalidCharactersInTag_xmsg, ">")); //$NON-NLS-2$
			}
			...
			if (!tagWasAdded) {
				return new Status(IStatus.ERROR, AbapTagsModelPlugin.PLUGIN_ID,
					NLS.bind(Messages.TagListValidator_DuplicateTagNameFound_xmsg, tag.getName().toUpperCase()));
			}
		}
